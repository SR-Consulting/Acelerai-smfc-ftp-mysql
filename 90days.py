"""
Carga de CSVs “*_90_days*.csv”
Vinícius – 27 jun 2025
"""
...
def dedup_case_insensitive(seq):
    """Remove duplicatas ignorando caso e espaços; preserva a primeira ocorrência."""
...
def check_and_create_columns(conn, cols):
    """Cria apenas colunas ausentes, ignorando o erro 1060 se ocorrer."""
    cur = conn.cursor()
    try:
        ...
        for col in cols:
            ...
            try:
                cur.execute(
                    f"ALTER TABLE {TABLE_NAME} " f"ADD COLUMN `{col}` VARCHAR(255)"
                )
...
def load_csv(csv_path: Path, pool):
    ...
    try:
        with csv_path.open(newline="", encoding=CSV_ENCODING) as f:
            ...
            for batch_no, rows in enumerate(chunked(rdr, BATCH_SIZE), start=1):
                ...
                try:  # captura erro do lote
                    ...
                    # salva lote problemático p/ análise
                    ...
                    conn.rollback()  # desfaz apenas o lote
                    # -> decide continuar ou re-lançar:
                    raise  # re-lança p/ main() pular p/ próximo CSV
                # --------------------------------------
        ...
        # aqui já foi feito rollback do lote; só garantir resto
        conn.rollback()
